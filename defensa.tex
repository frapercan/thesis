  ...
  \begin{columns}[t]
    ...
    \column{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item Examples: plugin split (D1), pgvector for storage only with
              NumPy/FAISS for search (ADR 001), contracts-first (D15)
      \end{itemize}
      \vspace{0.3em}
      \textbf{Observability}
      \begin{itemize}
...
\begin{frame}{LightGBM Re-Ranker: Per-Aspect to Full-Feature}
  \begin{columns}[t]
    \column{0.50\textwidth}
      \textbf{Per-aspect models (Exp.~4)}
      \begin{itemize}
      ...
      \end{itemize}
      \vspace{0.5em}
      \textbf{Per-category + IA weighting (Exp.~5)}
      \begin{itemize}
    ...
    \column{0.46\textwidth}
      \textbf{Full feature set, leakage-free (Exp.~6)}
      \begin{itemize}
...
\begin{frame}{Experiment 6: Full-Feature Re-Ranker Results}
  \begin{center}
    \begin{tabular}{l ccc ccc ccc}
      ...
      Per-category re-ranker$^\dagger$ & 0.425 & 0.607 & 0.689 & 0.486 & 0.575 & 0.707 & 0.199 & 0.297 & 0.335 \\
      \rowcolor{proteagreen!15}
      \textbf{Full-feature re-ranker} & \textbf{0.599} & \textbf{0.646} & \textbf{0.775} & \textbf{0.620} & \textbf{0.606} & \textbf{0.794} & 0.165 & 0.293 & 0.346 \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.3em}
  {\small $^\dagger$ Pre-leakage-fix dataset (22-feature schema, GOA 220$\to$229); not directly comparable with the full-feature row.}
  \vspace{0.3em}
  \begin{itemize}
    \item Full-feature re-ranker shows large NK and LK gains (leakage-free, full feature set)
    \item PK-BPO lower: leakage-fix tightens PK positive criterion
  \end{itemize}
\end{frame}

% ── Slide 17: v9 Study and cafaeval ─────────────────────────
\begin{frame}{Experiment 8: 56-Feature Re-Ranker and cafaeval Validation}
  \begin{columns}[t]
    \column{0.50\textwidth}
      \textbf{56-feature schema study (2026-05-01)}
      \begin{itemize}
      ...
      \end{itemize}
    \column{0.46\textwidth}
      \textbf{Selected cafaeval \fmax (56-feature):}
      \vspace{0.3em}
      \begin{tabular}{lrr}
        \toprule
        Cell & Lab & Cafaeval \\
      ...
      \end{tabular}
      \vspace{0.3em}
      {\small 56-feature vs KNN baseline: all 9 cells $p < 0.0001$ (paired bootstrap, 2000 resamples).}
  \end{columns}
\end{frame}
...
\begin{frame}{Feature Importance Analysis}
  \begin{columns}[t]
    \column{0.50\textwidth}
      \textbf{Top features by LightGBM gain (full-feature, seed 42)}
      \begin{itemize}
    ...
    \column{0.46\textwidth}
      \textbf{Leave-one-family-out ablation (56-feature, 3 cells)}
      \vspace{0.3em}
      \begin{tabular}{lr}
        \toprule
  ...
  \begin{columns}[c]
    \column{0.55\textwidth}
      \begin{tabular}{l ccc ccc}
        ...
        \rowcolor{proteagreen!15}
        \textbf{Full-feature} & \textbf{0.599} & \textbf{0.646} & \textbf{0.775} & \textbf{0.620} & \textbf{0.606} & \textbf{0.794} \\
        \bottomrule
      \end{tabular}
      \vspace{0.3em}
...
\begin{frame}{Leakage-Fixed Champion: Multiseed Binary Re-Ranker (LB.2/LB.3)}
  \begin{columns}[c]
    \column{0.55\textwidth}
      \textbf{Leakage-fixed binary recipe (anc2vec retrofix)}
      \begin{itemize}
        \item 3 seeds $\times$ 6 cells (NK+LK only)
        \item Selective avg: \highlight{0.6215 $\pm$ 0.0014} on v226
        \item Selective deploy: NK+LK $\to$ binary re-ranker, PK $\to$ KNN baseline
      \end{itemize}
      \vspace{0.5em}
  ...
  \begin{columns}[c]
    \column{0.55\textwidth}
      \begin{block}{CAFA~6 TEAM result}
        \begin{itemize}
          \item \highlight{Final ranking \#19} in the international CAFA~6 challenge
    ...
    \column{0.40\textwidth}
      ...
      \begin{itemize}
        \item CAFA~6 \#19 belongs to the research
...
% ── Slide 21b: LAFA Result ───────────────────────────────────
\begin{frame}{LAFA Continuous Benchmark: First-Place Result}
  \begin{columns}[t]
    \column{0.52\textwidth}
      \textbf{Official 7{,}401-target frame (Sep~2025 $\to$ Mar~2026)}
      \begin{itemize}
        \item IA-weighted micro-$F$, mean over NK/LK/PK
        \item Homology backbone + learned combiner (complementary predictor + cross-aspect association)
        \item Ingredients chosen on an earlier window, sealed once on the target frame
      \end{itemize}
      \vspace{0.4em}
      \begin{block}{Two figures, kept apart}
        \textbf{Offline research:} \highlight{0.391} (sealed champion, first place vs TransFew 0.381).
        Single-window measurement; original training artefacts no longer byte-reproducible.\\[0.3em]
        \textbf{Native productized:} \highlight{0.3745} (NK~0.4645 / LK~0.4526 / PK~0.2065).
        Reproduced end to end on the platform, served live from \texttt{/benchmark}.
      \end{block}
    \column{0.44\textwidth}
      \begin{tabular}{lr}
        \toprule
        Method & Mean \\
        \midrule
        PROTEA (offline research) & \highlight{0.391} \\
        TransFew & 0.381 \\
        FunBind & 0.366 \\
        \textbf{PROTEA (native productized)} & \textbf{0.3745} \\
        GOA non-exp. & 0.325 \\
        DeepGOPlus & 0.311 \\
        \bottomrule
      \end{tabular}
      \vspace{0.4em}
      {\small The platform serves and stands behind 0.3745; 0.391 is the sealed offline research result.}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[t]
    \column{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item $K \in \{3, 5, 10\}$
        \item leakage-fixed binary recipe per PLM
  ...
  \begin{columns}[t]
    ...
    \column{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Leakage-fixed binary selective: \highlight{0.6215 $\pm$ 0.0014}
        \item CAFA~6 \#19 (team result)
