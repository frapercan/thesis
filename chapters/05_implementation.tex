...
\begin{description}
  \item[\texttt{protea/api/}] FastAPI application and seventeen routers
    spanning the full operational surface of the platform. The routers are
    described in \cref{sec:api-design}.
  \item[\texttt{protea/core/}] Pure domain logic: the \texttt{Operation}
    protocol and \texttt{OperationRegistry} contracts, all registered
...
\begin{description}
  ...
  \item[\texttt{run\_cafa\_evaluation}] Runs the standalone \texttt{cafaeval} fork
    against a \texttt{PredictionSet} and persists \texttt{EvaluationResult} rows
    (Fmax, AuPRC, coverage, per-aspect breakdowns). Isolated on its own queue so
    long evaluations do not block the general jobs queue.
...
The web interface is a Next.js 16 application using Tailwind CSS v4 for styling.
