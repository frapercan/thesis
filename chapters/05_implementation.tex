...
term and the protein's pre-cutoff known annotation set, a signal that the v6
...
images for the LAFA v2 evaluation (ADR~D23), all layered on the
...
\begin{description}
  \item[\texttt{protea-knn-v1} (F-LAFA.1, PR~\#284)] The v1 baseline. Runs
    aspect-separated KNN with ProtT5-XL without v6 feature enrichment or a
    learned reranker (\texttt{-{}-no\_v6 -{}-no\_reranker}). Provides the
    kNN-only lower bound against which the v6 and ensemble submissions are
    measured.
  ...
  \item[\texttt{protea-knn-8plm} (F-LAFA.2, PR~\#294)] A score-level mean
    ...
    maximised via a CLI flag) before prediction. No v6 features or reranker.
    This image captures the embedding-level ensemble gain that the single-PLM
    v18 submission cannot achieve on its own.

  \item[\texttt{protea-v18} (F-LAFA.3, PR~\#305)] The full pipeline
    container: single PLM (ProtT5-XL UniRef50, half-precision encoder),
    aspect-separated KNN, complete v6 feature enrichment (NW/SW alignments,
