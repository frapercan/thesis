...
The headroom is therefore one of ranking, and part of it is self-inflicted. The
...
well and still leave scores mutually incomparable across proteins, which is
precisely the observed symptom: a global ranking quality of 0.79 by area under the
curve converting to an $f_{\mu,w}$ of only 0.126. Retraining the identical rows
and features with a pointwise, globally calibrated objective raises that area to
0.823 and $f_{\mu,w}$ to 0.152, a gain of $+0.026$ obtained without a single new
feature.

That correction is real but does not, on its own, close the gap, and the signal
that would is not yet identified. The two structural hypotheses testable with the
resources of this work are both eliminated by measurement: term co-occurrence as a
candidate generator ($+0.002$) and Gene Ontology hierarchical proximity as a
feature (area under the curve 0.550, decorrelated from the reranker yet
contributing $+0.000$ when blended). A text-aligned representation adds $+0.002$ on
this cell and a domain-signature graft is actively harmful on Biological Process,
while a larger base representation reorders the same candidates and so cannot help
where recall is not the constraint. The honest statement, and the one this work
makes, is that the frontier is characterised rather than crossed: its location is
known, its magnitude is quantified at roughly a fivefold ranking headroom inside
the existing shortlist, and the levers currently available are demonstrably
