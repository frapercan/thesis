...
\begin{figure}[htbp]
  ...
  \caption[Fmax as a function of k]{%
    ...
    Source: \texttt{figures/fig\_fmax\_vs\_k.py} @ \texttt{7078d54}
...
The v9 study described in Experiment~8 (\cref{subsec:exp8-reranker-v9}) was run
...
Their absolute $F_{\max}$ values are not directly comparable to the v3 numbers
reported here. A full leakage-free re-run of the baseline and heuristic
experiments is pending (lab-runner slice LB.2).
\end{quote}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l ccc ccc ccc}
    ...
    Re-ranker v1 (bal.)$^\dagger$   & 0.408 & 0.577 & 0.687 & 0.478 & 0.506 & 0.711 & 0.201 & 0.298 & 0.332 \\
    Re-ranker v2$^\dagger$          & 0.425 & 0.607 & 0.689 & 0.486 & 0.575 & 0.707 & 0.199 & 0.297 & 0.335 \\
    \textbf{Re-ranker v3} & \textbf{0.599} & \textbf{0.646} & \textbf{0.775} & \textbf{0.620} & \textbf{0.606} & \textbf{0.794} & \textbf{0.165} & \textbf{0.293} & \textbf{0.346} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
the v3 row, which uses \texttt{bench-v1-K5-filtered}
(52-feature schema, GOA~220$\to$230 eval split, mean of 3 seeds).

The v3 re-ranker trained on the leakage-free feature set achieves strong
...
rows is not valid; a full leakage-free re-run of the earlier methods is pending
(lab-runner slice LB.2).
...
\begin{figure}[htbp]
  ...
  \caption[Re-ranker progression across methods]{%
    ...
    aspect. The v3 bar reflects the leakage-free \texttt{bench-v1-K5-filtered}
    dataset (52 features, mean of 3 seeds); all other bars use the
    pre-leakage-fix \texttt{bench-v1-K5} dataset and are included for
    directional reference only. A full leakage-free re-run of earlier methods
    is pending (lab-runner LB.2).
    Source: \texttt{figures/fig\_reranker\_progression.py} @ \texttt{7078d54}
...
\begin{table}[htbp]
  \centering
  \caption{Top-10 features by LightGBM gain for v3 per-cell models (seed~42,
...
\begin{figure}[htbp]
  ...
  \caption[Top-10 LightGBM feature importances per category (v3, leakage-free)]{%
    ...
    Source: \texttt{figures/fig\_feature\_importance.py} @ \texttt{7078d54}
...
\subsubsection{Improvement Over v2 (Indicative)}

\Cref{tab:improvement} summarises the absolute $F_{\max}$ change of the
leakage-free v3 re-ranker relative to the pre-leakage-fix v2 baseline. These
deltas are not clean apples-to-apples comparisons because the datasets and
feature schemas differ; they are reported as an indicative reference pending
the full leakage-free ablation (lab-runner LB.2).

\begin{table}[htbp]
  \centering
  \caption[Indicative Fmax delta: v3 (leakage-free) vs.\ v2 (pre-leakage-fix)]{%
    Indicative $F_{\max}$ delta: v3 (leakage-free, \texttt{bench-v1-K5-filtered})
    vs.\ v2 (pre-leakage-fix, \texttt{bench-v1-K5}). Datasets and feature
...
leakage-free re-run of v2 is required to isolate the net feature contribution.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l ccc ccc ccc}
    ...
    \textbf{PROTEA re-ranker v3} & \textbf{0.599} & \textbf{0.646} & \textbf{0.775} & \textbf{0.620} & \textbf{0.606} & \textbf{0.794} & \textbf{0.165} & \textbf{0.293} & \textbf{0.346} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
The PROTEA re-ranker v3 outperforms both eggNOG-mapper and the pre-leakage-fix
baseline in NK and LK across all aspects. In PK-BPO the leakage-fix
recategorisation reduces the positive rate, leading to a lower absolute
$F_{\max}$ for v3 compared to the pre-leakage-fix baseline and eggNOG-mapper;
this is expected, as the leakage-free criterion is stricter. A side-by-side
comparison on a shared leakage-free eval set (including a re-run of
eggNOG-mapper) is pending (lab-runner LB.2).
...
Three caveats apply: (i)~the v3 and baseline rows use different evaluation
...
\begin{figure}[htbp]
  ...
  \caption[PROTEA vs eggNOG-mapper across NK/LK/PK and aspects]{%
    ...
    PROTEA's leakage-free re-ranker~v3 on the GOA~220$\to$230 evaluation
    set. The v3 and baseline rows use different datasets; direct cell-by-cell
    comparisons are indicative only. NK and LK show large margins over
    eggNOG-mapper across all methods. PK-BPO shows lower v3 performance
    due to the leakage-fix recategorisation tightening the positive set.
    Source: \texttt{figures/fig\_protea\_vs\_emapper.py} @ \texttt{7078d54}
...
% ── Experiment 8 ──────────────────────────────────────────────
\subsection{Experiment~8: Re-Ranker v9 Study (cafaeval Validation + Ablation)}
\label{subsec:exp8-reranker-v9}

Experiment~8 reports the study\_v9 campaign (2026-05-01), which trains the
...
assess variance. \Cref{tab:v9-replication} shows per-cell results and
means.

\begin{table}[htbp]
  \centering
  \caption[Re-ranker v9 replication: lab Fmax (3 seeds)]{Re-ranker v9 replication: lab $F_{\max}$ (3 seeds, 56-feature schema).}
  \label{tab:v9-replication}
  \resizebox{\textwidth}{!}{%
...
cafaeval (\texttt{prop=fill, norm=cafa}). \Cref{tab:v9-cafaeval} shows lab
and cafaeval $F_{\max}$ side by side with the ratio and absolute delta.

\begin{table}[htbp]
  \centering
  \caption[Re-ranker v9: lab vs cafaeval Fmax (winner per cell)]{Re-ranker v9: lab $F_{\max}$ vs cafaeval $F_{\max}$ for the winner per cell.}
  \label{tab:v9-cafaeval}
  \resizebox{\textwidth}{!}{%
...
distribution) confirm that the v9 re-ranker significantly outperforms the
raw KNN baseline in all nine cells (all $p < 0.0001$,
\cref{tab:v9-bootstrap}).

\begin{table}[htbp]
  \centering
  \caption[Re-ranker v9 bootstrap CIs vs KNN baseline (lab Fmax)]{Paired bootstrap: v9 vs KNN baseline (lab $F_{\max}$, seed 137 winner where applicable).}
  \label{tab:v9-bootstrap}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{l rr rr r r}
    \toprule
    \textbf{Cell}
      & \textbf{$F_{\max}$ v9} & \textbf{CI v9}
...
to $F_{\max}$. \Cref{tab:v9-ablation} reports $\Delta F_{\max} =
F_{\max}(\text{full}) - F_{\max}(\text{drop family})$; a larger positive value
means the family carries more signal.

\begin{table}[htbp]
  \centering
  \caption[Re-ranker v9 leave-one-family-out ablation]{Leave-one-family-out $\Delta F_{\max}$ ablation (v9, 56-feature schema).}
  \label{tab:v9-ablation}
...
PROTEA's leakage-free v3 re-ranker substantially exceeds both in NK and LK.
These comparisons are on different evaluation sets; a controlled comparison
on a shared leakage-free split is pending (lab-runner LB.2). The v9
...
Yes. The feature importance analysis for the leakage-free v3 re-ranker
...
category. The v9 leave-one-family-out ablation (Exp.~8,
\cref{tab:v9-ablation}) quantifies this: the \texttt{anc2vec\_query} family
...
The v3 leakage-free feature importance analysis (Tab.~\ref{tab:feature-importance})
and the v9 family ablation (Tab.~\ref{tab:v9-ablation}) together show that
...
top-10 in the v3 analysis. This confirms that feature engineering remains
...
\begin{itemize}
  ...
  \item The v9 hyperparameter grid reveals a 0.0283 spread on NK-BPO
