...
system fails to predict, 97\% use a term that already exists in the vocabulary
visible before the target window opens, so only 3\% are genuinely novel. Second,
retrieval is not what binds, even though the candidate pool recovers only 0.322 of
the true pairs. Scoring that same pool with the true label, which asks what a
perfect ordering of the candidates already retrieved would be worth, yields
$f_{\mu,w}=0.608$ where the reranker delivers 0.126: the system captures a fifth
of what its own shortlist allows. Third, and consequently, the candidate-side
