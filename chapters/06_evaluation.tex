...
re-ranker numbers reported here. A full leakage-free re-run of the baseline and heuristic
experiments (lab-runner LB.2) was not completed within the thesis timeline and
...
rows is not valid; a full leakage-free re-run of the earlier methods was not
completed within the thesis timeline and remains a known limitation (lab-runner LB.2).
...
\begin{figure}[htbp]
  ...
  \caption[Re-ranker progression across methods]{%
    ...
    (lab-runner LB.2) was not completed within the thesis timeline.
...
A controlled leakage-free ablation across all methods (lab-runner LB.2) was not
completed within the thesis timeline and remains a known limitation.
...
eggNOG-mapper, lab-runner LB.2) was not completed within the thesis timeline
...
The champion recipe (v27-binary: binary-label, generalist features, $k{=}5$)
is being replicated across eight canonical PLMs and three $K$ values
($K \in \{3, 5, 10\}$) to assess how the result generalises beyond ProstT5.
The FARM-EXP.13 export pipeline produces one benchmark dataset per
(PLM, $K$) cell, for a total of 24 datasets (\texttt{bench-v1-K\{3,5,10\}-v226-lineage-\{plm\}}),
covering ESM-2 in three sizes, ESMC-300M, ProstT5, ProtT5, Ankh-base, and
the ESM-3 component encoder.

This full (PLM $\times$ $K$) grid constitutes the empirical evidence base for
the generalisability claim: rather than selecting a winner per cell and
reporting only that winner (which would introduce a winner's-curse bias on the
v226-to-v230 generalisation window), all cafaeval results across the complete
grid are to be reported. At time of writing, 15 of the 24 (PLM, $K$) export
jobs have completed and the champion recipe reproduces the 0.7291 $\pm$ 0.0028
result across the completed cells. The remaining 9 cells (6 PLMs $\times$
$K \in \{3,5,10\}$) are deferred to a post-submission dataset package
(F-DATA-PACK) due to GPU resource constraints; completing them does not affect
the primary champion result, which is established on ProstT5. The multi-PLM
grid and F-DATA-PACK are discussed further in the limitations and future work sections.
...
on a shared leakage-free split (lab-runner LB.2) was not completed within the
thesis timeline and remains a known limitation. The 56-feature
...
\begin{itemize}
  ...
  \item The multi-PLM $\times$ $K$ generalisation grid (Exp.~9 / FARM-EXP.13)
    is partially complete: 15 of 24 export cells have completed at time of
    writing. The champion result (0.7291 $\pm$ 0.0028) is established on
    ProstT5 and reproduced across completed cells. The remaining 9 cells are
    deferred to the F-DATA-PACK post-submission dataset package; the thesis
    primary result does not depend on their completion.
\end{itemize}
