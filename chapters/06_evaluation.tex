...
\paragraph{Where the remaining headroom lives.}
The two unwon Biological Process cells are a limit of the ranking stage, not of the
available evidence. Four measurements locate it, each on one harness against the
full ground truth of the partly-known cell, on a booster retrained over the exported
dataset. That booster reproduces the deployed recipe at $f_{\mu,w}=0.213$ against the
benchmark's 0.218 on the same cell, so the figures below track the board closely.

First, the evidence is present: of the true partly-known protein to Biological
Process term pairs the system fails to predict, 97\% use a term that already exists
in the vocabulary visible before the target window opens, and only 3\% are genuinely
novel. Second, retrieval is not what binds, even though the candidate pool recovers
only 0.322 of the true pairs. Scoring that same pool with the true label, which asks
what a perfect ordering of the candidates already retrieved would be worth, yields
$f_{\mu,w}=0.608$ against the 0.213 the system delivers: it extracts about a third of
what its own shortlist allows. Consequently the candidate-side levers are inert, and
expanding the pool by term co-occurrence lifts recall by half, from 0.322 to 0.480,
for a small fraction of the gap, because additional candidates do not help a ranker
that cannot order the ones it already holds.

Third, the limit is not where the list is cut. A single global threshold cannot
express a per-protein term count: every protein is cut at the same point whether it
deserves three terms or thirty. Freezing the system's own ordering and granting each
protein its true count, an oracle no method could possess, moves $f_{\mu,w}$ from
0.202 to 0.238, which is $+0.036$ of a 0.406 gap, about a tenth. The other nine
tenths is ordering, and no clue the system carries separates the true candidates from
the rest: on this cell no feature exceeds an area under the curve of 0.68 against a
2.47\% positive rate.

Fourth, the technique levers available to this work are exhausted, because the
deployed recipe already sits at their optimum. Every variation tested scores below
it: training one model per cell rather than pooling the three namespaces reaches
0.202; a pointwise objective in place of the listwise one reaches 0.152; adding
within-protein rank and standardised features reaches 0.147; class weighting reaches
0.144; and pruning the classifier-proposed candidates from the pool leaves the score
unchanged while costing coverage. The pointwise result is worth isolating, because it
is the one a practitioner would expect to help and it carries a \emph{better} area
under the curve than the deployed recipe (0.823 against 0.790) together with a worse
$f_{\mu,w}$. Area under the curve orders these recipes against the metric that
decides, which is a warning this work records: ranking levers cannot be triaged by it.

The frontier is therefore characterised rather than crossed. Its location is known,
its magnitude is quantified at roughly a threefold ranking headroom inside the
existing shortlist, the structural hypotheses testable with these resources are
eliminated (term co-occurrence as a candidate generator; Gene Ontology hierarchical
proximity as a feature, at an area under the curve of 0.550 and contributing nothing
when blended), and the signal that would cross it is not identified.

Two methodological rules generalise beyond this cell and both are load-bearing here.
A recall number alone does not identify what binds a pipeline; the ceiling of the
pool does, and obtaining it costs one evaluation with the labels used as the score.
And a monotone rescaling of a score is not free under a threshold-swept metric:
because the sweep runs on a fixed grid, remapping the score distribution changes
which cuts the sweep can reach, and on this cell scoring one booster with its raw
output rather than with a global rank percentile of that output differs by 0.088. Any
transform applied before evaluation is part of the measurement.
