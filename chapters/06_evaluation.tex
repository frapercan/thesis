...
PROTEA places first overall, ahead of the previously leading learned systems. It
leads by a wide margin where no prior knowledge is available, draws level with the
best method where knowledge is limited, and trails only in the hardest setting,
the rare, newly assigned functions of partly known proteins, which is where the
remaining headroom now sits.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l rrr r}
    ...
    \textbf{Method} & \textbf{NK} & \textbf{LK} & \textbf{PK} & \textbf{Mean} \\
    \midrule
    \textbf{PROTEA (this work)}    & \textbf{0.477} & \textbf{0.482} & \textbf{0.215} & \textbf{0.391} \\
    TransFew                       & 0.428 & 0.485 & 0.230 & 0.381 \\
    FunBind                        & 0.441 & 0.451 & 0.205 & 0.366 \\
    GOA non-experimental           & 0.426 & 0.390 & 0.158 & 0.325 \\
...
as a self-contained, reproducible package in the project's offline laboratory, and
the sealed result is served from the \texttt{/benchmark} surface alongside the
homology-only figure of Experiment~12, marked with the LAFA frame and the sealed
($227 \to 230$) window. As elsewhere in this chapter, the application is the single
source of truth for the reported figure.
