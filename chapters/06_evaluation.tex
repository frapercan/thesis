...
identity~$=0.3$, SW identity~$=0.2$) improves the baseline by 1.5--4\%
...
The v1 re-ranker improves CCO substantially (+2--4\% over baseline) but
\emph{degrades} MFO by 3--9\% relative to the heuristic. Six of nine per-aspect
...
\begin{itemize}
  ...
  \item \textbf{Alignment features are now active}: \texttt{identity\_nw} and
    \texttt{similarity\_nw} rank 4th--6th across all models. In v2 (where
...
statistics on top of embeddings provides a consistent 1.5--4\% improvement,
