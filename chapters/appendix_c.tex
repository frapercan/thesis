...
A universal versioned path prefix is applied to all routers (currently
\texttt{/v}\emph{N}\texttt{/}). Future breaking changes introduce a new major
version without removing existing endpoints. \texttt{Accept}-header negotiation
is deferred unless a concrete need surfaces. Deprecated unprefixed mounts are
kept for one release cycle to avoid breaking live clients (the
\texttt{protea.ngrok.app} deployment and existing front-end fetchers) during
the transition.
...
at the initial versioned-prefix transition, and are thereafter insulated from
internal restructuring under the same major version.
...
% ────────────────────────────────────────────────────────────
\section{D10: \texttt{schema\_sha} Parallel Column Migration}
\label{app:adr-d10}
...
the 56-feature re-ranker study on 2026-05-01 and caused a non-reproducible run.
...
read path switches to the new \texttt{schema\_sha\_v2} column; the legacy column
is retained until the F3 gate for audit purposes and then dropped. A regression
test exposes the parity drift on historical rows rather than retroactively
modifying them.
